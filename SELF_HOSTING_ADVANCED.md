...
| `AUTH_TOKEN_TTL` | Browser session lifetime for the `multica_auth` and `multica_csrf` cookies. Accepts Go duration strings (`720h`, `8760h`) or integer seconds. | `720h` |
| `MULTICA_SESSION_TTL` | **Deprecated** — has no effect on cookie MaxAge. Use `AUTH_TOKEN_TTL` instead. | — |
